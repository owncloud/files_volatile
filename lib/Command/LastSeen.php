...
class LastSeen extends Base  {
	...
	protected function configure() {
		parent::configure();
		$this
			->setName('user:lastseen-volatile')
	...
	protected function execute(InputInterface $input, OutputInterface $output) {
		...
		$order = strtoupper($input->getOption('order'));
				...
				if ($outputFormat === self::OUTPUT_FORMAT_PLAIN ) {
					...
					$users = [
						[
							...
							// storage stats
							'free' => $home->getFreeSpace(),
							'used' => $home->getSize(),
							'volatile' => $volatile->getSize(),
						]
					];
				}
